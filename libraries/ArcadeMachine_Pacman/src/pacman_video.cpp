...
// pacman_video_render_scanline() below, same as invaders_video.cpp/
...
static uint16_t frame_cache[PACMAN_GAME_HEIGHT][PACMAN_GAME_WIDTH];
...
void pacman_draw_frame(pacman_system *system) {
    ...
    if (system->rotation == 0 || system->rotation == 2) {
        for (uint32_t y = 0; y < (uint32_t)PACMAN_GAME_HEIGHT; y++)
